-- Permite salvar varios funis especificos por usuario mantendo compatibilidade
-- com a assinatura anterior que enviava apenas um funnel_id.

DROP FUNCTION IF EXISTS public.set_user_funnel_scope(uuid, boolean, uuid);
DROP FUNCTION IF EXISTS public.set_user_funnel_scope(uuid, boolean, uuid[]);
DROP FUNCTION IF EXISTS public.set_user_funnel_scope(uuid, boolean, uuid[], uuid);
...
SET search_path = public
AS $$
  SELECT public.set_user_funnel_scope_impl(
    _target_user_id,
    _has_all_funnel_access,
    _funnel_ids
  );
$$;
...
AS $$
  SELECT public.set_user_funnel_scope_impl(
    ...
    _has_all_funnel_access,
    CASE
      WHEN _funnel_id IS NULL THEN NULL::uuid[]
      ELSE ARRAY[_funnel_id]
    END
  );
$$;
